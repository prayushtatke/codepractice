...
function totalRibbon(input) {
    ...
    return input
        ...
        .filter((e) => e !== "")
        .map((present) => present.split("x").map(Number))
        .map((dims) => getRibbonLength(...dims))
        .reduce((acc, curr) => acc + curr);
}

let input = utils.readInput("./part1_input.txt");
console.log("Part-1:", totalWrappingPaper(input));

input = utils.readInput("./part2_input.txt");
console.log("Part-2:", totalRibbon(input));
